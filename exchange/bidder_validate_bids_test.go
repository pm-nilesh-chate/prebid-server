...
func TestAllValidBids(t *testing.T) {
	...
	seatBids, errs := bidder.requestBid(context.Background(), bidderReq, bidAdjustments, currency.NewConstantRates(), &adapters.ExtraRequestInfo{}, true, false, nil)
...
func TestCurrencyBids(t *testing.T) {
	...
	for _, tc := range currencyTestCases {
		...
		bidAdjustments := map[string]float64{string(openrtb_ext.BidderAppnexus): 1.0}
		seatBids, errs := bidder.requestBid(context.Background(), bidderRequest, bidAdjustments, currency.NewConstantRates(), &adapters.ExtraRequestInfo{}, true, false, nil)
		assert.Len(t, seatBids, 1)
...
func (b *mockAdaptedBidder) requestBid(ctx context.Context, bidderRequest BidderRequest, bidAdjustments map[string]float64, conversions currency.Conversions, reqInfo *adapters.ExtraRequestInfo, accountDebugAllowed, headerDebugAllowed bool, alternateBidderCodes *openrtb_ext.ExtAlternateBidderCodes) ([]*pbsOrtbSeatBid, []error) {
